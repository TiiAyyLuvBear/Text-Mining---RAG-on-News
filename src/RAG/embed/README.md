...
Module này dùng các chunk đã tạo trong `data/chunking/output` để xây dense index cho thí nghiệm retrieval. Toàn bộ output mặc định được ghi trong `data/embed/output`.
...
```powershell
python -m src.RAG.embed.embed_chunks `
  --input data/chunking/output/vieonline_news_chunks_token.jsonl `
  --output-dir data/embed/output/dense/token `
...
```powershell
python -m src.RAG.embed.dense_search `
  --index-dir data/embed/output/dense/token `
...
```powershell
python -m src.RAG.embed.compare_embedding_inputs `
  --inputs data/chunking/output/vieonline_news_chunks_token.jsonl `
           data/chunking/output/vieonline_news_chunks_langchain_recursive.jsonl `
           data/chunking/output/vieonline_news_chunks_llamaindex.jsonl `
           data/chunking/output/vieonline_news_chunks_structured.jsonl `
  --sample-articles 3 `
  --output data/embed/output/embedding_strategy_samples.md
```
...
Notebook `src/RAG/embed/embedding_pipeline.ipynb` chạy tuần tự bằng các CLI hiện có:

1. `src.RAG.data_ingestion.cli` để tạo JSONL sạch.
2. `src.RAG.chunking.cli` để tạo chunk theo strategy.
3. `src.RAG.embed.compare_embedding_inputs` để xem mẫu khác biệt giữa strategy.
4. `src.RAG.embed.embed_chunks` để embed từng strategy với `tqdm` progress.
5. Đọc `embedding_stats.json` và lập bảng tổng hợp tại `data/embed/output/embedding_stats_summary.csv`.
...
```text
data/embed/output/eval/leaderboard_qa50.csv
data/embed/output/eval/per_query_results_qa50.jsonl
```
