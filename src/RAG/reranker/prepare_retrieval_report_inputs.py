                ...
                candidates.append(
                    {
                        ...
                        # Keep serving metadata. The offline reranker output can then
                        # become RAG contexts without another chunk lookup.
                        "title": (chunk.get("metadata") or {}).get("title", chunk.get("title", "")),
                        "category": (chunk.get("metadata") or {}).get("category", chunk.get("category", "")),
                        "url": (chunk.get("metadata") or {}).get("url", chunk.get("url", "")),
                        "chunk_index": (chunk.get("metadata") or {}).get("chunk_index", chunk.get("chunk_index", 0)),
                    }
                )
