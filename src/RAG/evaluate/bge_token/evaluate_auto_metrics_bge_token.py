...
def main():
    parser = argparse.ArgumentParser(description="Evaluate BGE token generated QA answers with BLEU, ROUGE-L, and BERTScore.")
    parser.add_argument("--pred", default="data/generation/output/answers_token_bge_top5_claude.jsonl")
    parser.add_argument("--gold", default="Dataset/QA_Claude/QA_output.jsonl")
    parser.add_argument("--out-dir", default="data/evaluate/output/bge_token")
    ...
    write_json(out_dir / "auto_metrics_bge_token_summary.json", summary)
    write_jsonl(out_dir / "auto_metrics_bge_token_pairs.jsonl", per_question)
