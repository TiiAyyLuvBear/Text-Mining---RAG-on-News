...
Module này tạo các biến thể chunking cho tập VieOnlineNews và ghi toàn bộ kết quả vào `data/chunking/output`.
...
python -m src.RAG.chunking.cli `
  ...
  --output-dir data\chunking\output\subdataset `
  --chunk-size 45 `
  --overlap 8 `
...
| --- | ---: | --- |
| `--input` | `data/processed/vieonline_news_clean.jsonl` | JSONL đã preprocess từ `src.RAG.data_ingestion`. |
| `--output-dir` | `data/chunking/output` | Thư mục ghi file chunk JSONL, summary và report. |
