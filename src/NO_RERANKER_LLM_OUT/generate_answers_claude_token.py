...
DEFAULT_OUTPUT = "src/NO_RERANKER_LLM_OUT/answers_token_top5_claude_no_reranker.jsonl"
...
def build_context(candidates):
    ...
    for candidate in candidates:
        block = (
            ...
            "Retrieval score: " + str(candidate.get("score")) + "\n"
...
def parse_args():
    parser = argparse.ArgumentParser(description="Generate answers from embedding retrieval output without reranker.")
...
def main():
    ...
    for index, row in enumerate(read_jsonl(input_path), start=1):
        ...
        top_contexts = row.get("candidates", [])[: args.top_n_context]
