...
def score_pairs(model: Any, tokenizer: Any, pairs: List[List[str]], batch_size: int, max_length: int) -> List[float]:
    ...
    for batch in batch_items(pairs, batch_size):
        # Mã hóa chuỗi text, giới hạn chiều dài ngữ cảnh đầu vào
        inputs = tokenizer(
            list(batch), 
            padding=True, 
            truncation=True, 
        ...
        with torch.no_grad():
            ...
            # Dùng hàm Sigmoid đưa giá trị điểm số Logits thô về dải xác suất chuẩn 0-1
...
def rerank_row(
    ...
    # Tích hợp thêm bộ phân tách từ tokenizer vào pipeline tính điểm số
    ...
    return {
        ...
        "embedding_type": "token",  # Giữ đúng nhãn định danh chiến thuật token
        "reranker": model_name,
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="Rerank token embedding output with Jina reranker.")
    parser.add_argument("--input", default=DEFAULT_INPUT, help="Input per-query token JSONL file.")
    ...
    parser.add_argument("--batch-size", type=int, default=16, help="Pair scoring batch size. Đẩy lên 16 tối ưu GPU")
...
def main() -> None:
    ...
    # --- ĐẠI TU ĐOẠN KHỞI TẠO MÔ HÌNH TRÊN GPU ---
    print("--> Đang nạp Tokenizer cho chiến thuật Token...")
    tokenizer = AutoTokenizer.from_pretrained(args.model_name, trust_remote_code=True)
    
    print("--> Đang nạp Jina Reranker V2 với cấu hình ép kiểu dữ liệu nửa chính xác float16...")
    model = AutoModelForSequenceClassification.from_pretrained(
        args.model_name, 
        trust_remote_code=True,
        use_flash_attn=False,       # Ngăn chặn log rác và treo hệ thống
        torch_dtype=torch.float16   # Ép kiểu float16 tối ưu hóa tốc độ trên T4 card
    )
    
    # Đẩy toàn bộ kiến trúc mô hình lên chip GPU tăng tốc
    device = "cuda" if torch.cuda.is_available() else "cpu"
    model.to(device)
    print(f"--> Khởi tạo hoàn tất! Thiết bị xử lý hiện hành: {device.upper()}")
