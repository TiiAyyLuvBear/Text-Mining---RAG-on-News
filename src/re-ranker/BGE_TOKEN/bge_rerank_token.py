...
DEFAULT_INPUT = "src/embed/output/dense/per_query_token.jsonl"
DEFAULT_OUTPUT = "src/re-ranker/out_reranker/rerank_token_bge_top5.jsonl"
...
def score_pairs(reranker: FlagReranker, pairs: List[List[str]], batch_size: int) -> List[float]:
    scores: List[float] = []
    for batch in batch_items(pairs, batch_size):
        batch_scores = reranker.compute_score(list(batch))
...
def rerank_row(
    ...
    scores = score_pairs(reranker, pairs, batch_size) if pairs else []
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="Rerank token embedding output with BGE reranker.")
    ...
    parser.add_argument("--model-name", default=DEFAULT_MODEL, help="BGE reranker model name.")
    ...
    parser.add_argument("--fp16", action="store_true", help="Use fp16. Recommended on CUDA GPU.")
...
def main() -> None:
    ...
    reranker = FlagReranker(args.model_name, use_fp16=args.fp16)
    ...
    for index, row in enumerate(rows, start=1):
        if args.limit is not None and index > args.limit:
            break
        outputs.append(rerank_row(row, reranker, args.model_name, args.top_n, args.batch_size))
        if index % 10 == 0:
