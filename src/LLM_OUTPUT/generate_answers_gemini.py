"""Generate BGE-structure RAG answers with the native Gemini API."""
import argparse
import json
import os
import time
...
DEFAULT_OUTPUT = "src/LLM_OUTPUT/answers_structure_bge_top5_gemini.jsonl"
...
def call_llm(api_key, base_url, model, question, contexts, max_tokens, retries, retry_sleep, timeout):
    ...
    endpoint = base_url.rstrip("/") + "/models/" + quote(model, safe="") + ":generateContent"
    last_error = None
    for attempt in range(1, retries + 1):
        try:
            ...
            if not response.ok:
                raise RuntimeError(
                    "Gemini API " + str(response.status_code) + ": " + response.text[:500]
                )
...
def parse_args():
    parser = argparse.ArgumentParser(
        description="Generate structured BGE rerank answers with Gemini."
    )
