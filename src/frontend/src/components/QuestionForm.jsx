...
const examples = [
  "Tại sao nước dùng hầm xương hoặc nước lẩu lại có thể gây hại cho thận?",
  "Loại nội tạng nào nên hạn chế để tránh tăng axit uric?",
  "Nhà tù Hỏa Lò có ý nghĩa gì đối với du lịch Việt Nam?"
];

export function QuestionForm({ question, setQuestion, topK, setTopK, onSubmit, loading }) {
  return <section className="question-panel" aria-labelledby="ask-title"><div><p className="kicker">Đặt câu hỏi</p><h2 id="ask-title">Bạn muốn tìm hiểu điều gì hôm nay?</h2></div>
    ...
    </form><div className="examples" aria-label="Câu hỏi gợi ý">{examples.map((item) => <button type="button" key={item} onClick={() => setQuestion(item)} disabled={loading}>{item}</button>)}</div>{loading && <StatusSteps />}
  </section>;
}
