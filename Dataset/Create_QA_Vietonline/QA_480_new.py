...
INPUT_CSV      = os.path.join(SCRIPT_DIR, "VietOnlineNews", "train_new.csv")
OUTPUT_JSONL   = os.path.join(SCRIPT_DIR, "QA_output_new_480.jsonl")
...
MAX_SINGLE_CONTEXT_CHUNKS = 8
MAX_CROSS_CONTEXT_CHUNKS_PER_ARTICLE = 4
...
def generate_cross_article_qa(articles: list[dict], num_questions: int = 2) -> list[dict]:
    ...
    response = client.chat.completions.create(
        ...
        messages=[
            ...
            {"role": "system", "content": CROSS_DIVERSITY_PROMPT},
...
def generate_qa(article_id: int, title: str, content: str) -> list[dict]:
    ...
    response = client.chat.completions.create(
        ...
        messages=[
            ...
            {"role": "system", "content": SINGLE_DIVERSITY_PROMPT},
...
def main():
    ...
    df = candidates.head(PHASE1_ARTICLE_COUNT).copy()
