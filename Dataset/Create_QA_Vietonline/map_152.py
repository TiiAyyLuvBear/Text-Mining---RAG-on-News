...
DEFAULT_QA_PATH = BASE_DIR.parent / "QA_Claude" / "QA_output.jsonl"
...
def source_article_ids(record: dict) -> list[str]:
    ...
    # Older Claude output stores cross-article IDs as one string such as
    # "150854; 150858; 150614" instead of a JSON array. Expand both formats.
    expanded_ids = []
    for value in raw_ids:
        if isinstance(value, str) and re.search(r"[;,|]", value):
            expanded_ids.extend(re.split(r"\s*[;,|]\s*", value.strip()))
        else:
